...
def parset_mixin(parset: dict, mixin: dict) -> bytes:
    ...
    Returns
    -------
        The binary encoded YANDA parset. 
    """
...
# Merge dictionaries (template)
def merge_dictionaries(static_dict:dict, dynamic_dict:dict)-> dict: 
    """
    Merge two dictionaries.
    
    Parameters
    ----------
    static_dict: The first dictionary. 
    dynamic_dict: The second dictionary.
...
def Cimager_test(input_dict:dict):
    """
    Print each key-value pair in the input dictionary.

    Parameters
    ----------
    input_dict: The dictionary containing key-value pairs to be printed.
...
def combine(str1:str, str2:str)-> str: 
    """
    Combine two input strings representing key-value pairs, removing conflicts and empty values.
    
    Parameters
    ----------
    str1: The first string, with lines in "key=value" format.
    str2: The second string, also with lines in "key=value" format.
    
    Returns
    -------
        The combined key-value pairs as a single string, with conflicts resolved.
    """ 
...
# Mixing parsets 
def parset_mixin_2(parset: dict, mixin: list, prefix:str="") -> bytes:
    """
    Update parset with dict values.
...
def parset_mixin_gayatri(parset: dict, mixin: list) -> bytes:
    """
...
def calculate_difference_image(given_image, reference_image, start_channel:int, end_channel:int):
    """
    ...
    start_channel: The starting channel number (inclusive). 
    end_channel: The ending channel number (inclusive).

    Returns
    -------
        The difference image (numpy array). 
    """
...
def tap_query_from_csv(test_catalogue:str)-> list:
    """
    Reads a CSV file and performs a TAP query for each entry.
    
    Parameters
    ----------
    test_catalogue: The path to the CSV file.
    
    Returns
    -------
        A list of query results for each entry in the CSV file.
    """
    results = []
...
def download_file(url:str, check_exists:bool, output:str, timeout:int, buffer=4194304)->str:
    ...
    - timeout: The maximum time (in seconds) to wait for a server response.
...
def process_and_download_data_same_folder(credentials:str, input_csv:str, processed_catalogue:str, timeout_seconds:int):
    """
    Processes an input catalogue to check for existing sources, queries unprocessed sources, 
    retrieves relevant data, stages files for download, and saves the processed details to a CSV file 'hipass_ms_file_details.csv' in the working directory.
...
def imager():
    """
    Generates a unique filename for the imager output with a 'image_N.fits' format, creates the file and prints a confirmation message with the filename created.
...
def imcontsub():
    """
    Generates a unique filename for the imcontsub output with a 'image_N.contsub.fits' extension, creates the file and prints a confirmation message with the filename created.
...
def linmos():
    """
    Generates a unique filename for the imcontsub output with a 'image_N.contsub_holo.fits' extension, creates the file and prints a confirmation message with the filename created.
...
def process_and_download(credentials:str, input_csv:str, processed_catalogue:str, timeout_seconds:int, project_code:str):
    """
    Processes an input catalogue to check for existing sources, queries unprocessed sources, 
    retrieves relevant data, stages files for download, and saves the processed details to a CSV file 'hipass_ms_file_details.csv' in the working directory.
...
# Function to filter out un-processed sources
def filter_unprocessed_data(input_csv:str, processed_catalogue:str):
    """
    Processes an input catalogue to filter out already processed sources and save the unprocessed sources to a new CSV file 'test_catalogue_unprocessed.csv' in the working directory.
    ...
    """
    # Load the processed catalogue to check for already processed sources
    processed_df = pd.read_csv(processed_catalogue)
    processed_sources = set(processed_df['Name'])  # Convert to set for faster lookups

    # Load the input CSV into a DataFrame
    input_df = pd.read_csv(input_csv)

    # Filter the input DataFrame to exclude processed sources
    unprocessed_df = input_df[~input_df['Name'].isin(processed_sources)]

    # Output the unprocessed sources to a new CSV file
    output_csv = "filtered_sources.csv"
    unprocessed_df.to_csv(output_csv, index=False)
    print(f"Unprocessed sources saved to {output_csv}")
...
def process_data_1(credentials:str, input_csv:str):
    """
    Processes an input catalogue of unprocessed sources to retrieve relevant data, saves the processed details to a CSV, and outputs the results to 'hipass_ms_file_details.csv' in the working directory.
    ...
    with open(input_csv, mode='r') as csv_file:
        ...
        for row in csv_reader:
            ...
            for obs_id, visibility in zip(map(str, res['obs_id']), map(str, res['filename'])):
                sbid_visibility_dict.setdefault(obs_id, []).append(visibility)

            # Update sbid_visibility_dict (modifying the keys)
            sbid_visibility_dict = {key.replace('ASKAP-', ''): value for key, value in sbid_visibility_dict.items()}

            # Step 2: Create sbid_evaluation_dict from sbid_visibility_dict
            sbid_evaluation_dict = {}

            # For every unique SBID
            for sbid in sbid_visibility_dict.keys():
                # Run the TAP query for the current SBID
                res = tap_query_sbid_evaluation(sbid)
            ...
            # Step 3: Get RA, DEC, and Vsys from the query
            res = tap_query_RA_DEC_VSYS(name) 
    ...
    # Step 5 Creates a df with with filename, RA, DEC and System Velocity 
    output_df = pd.DataFrame(output_data, columns=['Name', 'RA', 'DEC', 'Vsys'])

    # Step 6 Add an additional column i.e. the evaluation file, using apply to create the new column 
    output_df['evaluation_file'] = output_df['Name'].apply(find_evaluation_file, args=(updated_vis_eval_dict,))
    ...
    # Create a new column evaluation_file_path by combining evaluation_file with the suffix
    output_df['evaluation_file_path'] = output_df['evaluation_file'].apply(lambda x: None if pd.isnull(x) else x.replace('.tar', f"/{suffix}"))
...
def download_data_ms_1(credentials:str, input_csv:str, timeout_seconds:int):
    """
    Downloads and untars the .ms files for a given HIPASS source. 
    ...
    print(f".ms files downloaded and un-tarred")

def download_data_eval_1(credentials:str, input_csv:str, project_code:str):
    """
    Downloads and untars the evaluation files for a given HIPASS source. 
    ...
    with open(input_csv, mode='r') as csv_file:
        ...
        for row in csv_reader:
            name = row['Name']
            print(f"Downloading evaluation files for: {name}")
    ...
    print(f"evaluation files downloaded and un-tarred!")

def download_data_ms_returns_urls(credentials:str, input_csv:str, processed_catalogue:str, timeout_seconds:int, project_code:str) -> list:
    """
    Downloads and untars the .ms files for a given HIPASS source. 
    ...
    print(f"url_list received.")
    return url_list

def download_data_ms_url(download_url:str, timeout_seconds:int):
    """
    Downloads and untars the .ms files for a given HIPASS source from a given URL.

    Parameters
    ----------
    download_url: The URL to be downloaded.
    timeout_seconds: Timeout setting in seconds for download operations.
    ...
    # Untar the file if it is a tar archive
    if downloaded_file.endswith('.tar') and tarfile.is_tarfile(downloaded_file):
        print(f"Untarring file: {downloaded_file}")
        untar_file(downloaded_file, '.')

    print(f"{download_url} downloaded and untarred successfully.")